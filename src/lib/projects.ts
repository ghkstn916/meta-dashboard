...
export const projects: Project[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { id: "lesson-2026ai_api_connect", name: "🔌 내 작품에 API 붙이기", desc: "작품을 만들던 학생이 API·서버·DB를 붙이는 단계로 넘어가는 단일 실습 페이지 8단계 — API 개념/공개 API 첫 호출(Open-Meteo 실시간 데모)/키와 CORS/Vercel 서버리스 함수/Claude API로 AI 기능 넣기(학교 프록시 데모 내장)/Supabase 테이블·RLS/전체 구조/제출 전 점검표. 웹(fetch)·파이썬(Colab) 두 트랙 토글. 코드 블록마다 복사 버튼.", tech: "React", url: "https://lesson-2026aiapiconnect.vercel.app", group: "인공지능 기초" },
